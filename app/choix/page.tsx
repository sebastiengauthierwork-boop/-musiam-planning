...
export default function ChoixPage() {
  ...
  // Supprimer tout flag de mode précédent au chargement
  useEffect(() => {
    try { sessionStorage.removeItem('just_logged_in') } catch {}
  }, [])

  // Garde d'accès : non connecté → login, salarié → mon-planning
  useEffect(() => {
    if (loading) return
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-6 py-10"
      ...
      <div className="w-full max-w-sm">
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-gray-700 mt-2">Que souhaitez-vous faire ?</p>
        </div>
